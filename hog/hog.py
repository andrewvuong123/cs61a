...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            ...
            score0 += take_turn(a, score1, select_dice(score0, score1))
        if player == 1:
            b = strategy1(score1,score0) 
            score1 += take_turn(b, score0, select_dice(score1, score0))
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    test = 1
    while test <= 10:
        temp_func = make_averaged(roll_dice)
        current = temp_func(test,dice)
        # current = make_averaged(roll_dice)(test, dice)
        if current > highest:
            highest = current
            num_rolls = test 
        test += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    if bacon_strategy(score, opponent_score, margin=8, num_rolls=4) == 0:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 11
    roll0 = take_turn(0, opponent_score)
    margin = 11
    if score + roll0 == 2 * opponent_score:
        return 5
    if score + roll0 >= 100:
        return 0
    if score >= 94:
    ...
    if score >= 84:
        return 4
    if 2 * take_turn(0, opponent_score) == opponent_score:
        return 0
    if roll0 >= margin:
        return 0
