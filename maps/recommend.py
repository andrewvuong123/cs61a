...
def find_closest(location, centroids):
    ...
    min_cen = centroids[0]
    for x in centroids:
        if distance(location, x) < min_dist:
            min_cen, min_dist = x, distance(location, x)
    return min_cen
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    for x in restaurants:
        closest = find_closest(restaurant_location(x), centroids)
        clusters.append([closest, x])
...
def find_centroid(cluster):
    ...
    longitude = []
    for x in cluster:
        latitude.append(restaurant_location(x)[0])
        longitude.append(restaurant_location(x)[1])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for x in feature_fns:
        feature.append(find_predictor(user, reviewed, x))
    return max(feature, key=lambda feature: feature[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            rating[name] = user_rating(user, name)
        else:
            rating[name] = predictor(restaurant)
    return rating
    # END Question 9
